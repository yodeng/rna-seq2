...
parser=OptionParser(description='Generates two txt files containing the count matrices for genes and transcripts, using the coverage values found in the output of `stringtie -e`, if input a directory, only $input_dir/*/transcripts.gtf will be match')
...
parser.add_option('-g', default='gene_count_matrix.txt', help="where to output the gene count matrix [default: %default")
parser.add_option('-t', default='transcript_count_matrix.txt', help="where to output the transcript count matrix [default: %default]")
...
if (os.path.isfile(opts.input)):
    ...
    samples = [(i,glob.glob(os.path.join(opts.input,i,"transcripts.gtf.filter.gtf"))[0]) for i in next(os.walk(opts.input))[1] if re.search(opts.pattern,i) and len(glob.glob(os.path.join(opts.input,i,"transcripts.gtf.filter.gtf")))]   ### 仅匹配"transcripts.gtf"文件
...
##                        transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
                    t_dict.setdefault(t_id, defaultdict(int))
...
##        print i,v
        geneDict.setdefault(geneIDs[i],defaultdict(int)) #gene_id
        geneDict[geneIDs[i]].setdefault(s[0],0)
        geneDict[geneIDs[i]][s[0]]+=v.setdefault(s[0],0)
...
#samplelist = [x for x,y in samples]
#for k in t_dict:
#    if len(t_dict[k]) != len(samplelist):
#        for sn in samplelist:
#            t_dict[k].setdefault(sn,0)
#for k in geneDict:
#    if len(geneDict[k]) != len(samplelist):
#        for sn in samplelist:
#            geneDict[k].setdefault(sn,0)

with open(opts.t, 'w') as t_outfile:
    t_outfile.write("transcript_id\t" + "\t".join([x for x,y in samples]) +"\n")
    for i in t_dict:
        t_outfile.write(i + "\t" + "\t".join([str(t_dict[i][s]) for s,v in samples]) + "\n")

with open(opts.g, 'w') as g_outfile:
