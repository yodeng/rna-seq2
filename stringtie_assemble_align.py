...
parser.add_argument("-anno","--anno",type=str,help="the genome anno file",required = True)
...
def write_assemble_makeflow(makeflow,samples,aligndir,assembledir,gtf,thread,strand):
    ...
    for sn in samples:
        if not os.path.exists(os.path.join(aligndir,sn,"accepted_hits.bam")):
            ...
            fo.write("&> %s && rm -fr %s\n\n" %(os.path.join(aligndir,sn,"sort.log"),os.path.join(aligndir,sn,"accepted_hits.sam")))
...
def write_filter_gtf_makeflow(makeflow,assembledir,samples):
    ...
    fo.write("\tpython %s %s &> %s\n\n"%(filter_gtf," ".join([os.path.join(assembledir,sn,"transcripts.gtf") for sn in samples]),os.path.join(assembledir,"filter_gtf.log")))
    fo.close()
...
def write_summary_trans_stat_makeflow(makeflow,samples,assembledir):
    ...
    fo.write("CATEGORY=summary_transcripts_len_png\n")
    fo.write(os.path.join(assembledir,"merge","merged.gtf.transcript.length_distribution.png : ") + os.path.join(assembledir,"merge","merged.gtf.transcript.length_distribution\n"))
    fo.write("\tRscript /lustre/work/yongdeng/software/protokaryon/flow/Length_distribution_bar_plot.r %s %s &> %s\n\n"%(os.path.join(assembledir,"merge","merged.gtf.transcript.length_distribution"),os.path.join(assembledir,"merge","merged.gtf.transcript.length_distribution.png"),os.path.join(assembledir,"merge","Length_distribution_bar_plot.log")))
    fo.close()
...
def write_trans_length_exon_num(makeflow,gtf,outputdir,trans_count):
    fo = open(makeflow,"a+")
    fo.write("CATEGORY=Transcript_length_exonNum\n")
    fo.write(os.path.join(outputdir,"Transcript_mRNA_length_exonNum.txt : ") + gtf + " " + trans_count + "\n")
    fo.write("\tpython /lustre/work/yongdeng/software/eukaryon/flow/Transcript_length_exonNum.py -gtf %s -id %s -o %s &> %s\n\n"%(gtf,trans_count,os.path.join(outputdir,"Transcript_mRNA_length_exonNum.txt"),os.path.join(outputdir,"Transcript_mRNA_length_exonNum.log")))
    fo.write("CATEGORY=Transcript_length_exonNum_plot\n")
    fo.write(os.path.join(outputdir,"Transcript_exonNum.boxplot.png ")+ os.path.join(outputdir,"Transcript_Log2exonNum.boxplot.png ") + os.path.join(outputdir,"Transcript_length.boxplot.png ") + os.path.join(outputdir,"Transcript_Log2length.boxplot.png : ") + os.path.join(outputdir,"Transcript_mRNA_length_exonNum.txt\n"))
    fo.write("\tRscript /lustre/work/yongdeng/software/eukaryon/flow/draw_plot.r " + os.path.join(outputdir,"Transcript_mRNA_length_exonNum.txt ") + os.path.join(outputdir,"Transcript_exonNum.boxplot.png ") + os.path.join(outputdir,"Transcript_Log2exonNum.boxplot.png ") + os.path.join(outputdir,"Transcript_length.boxplot.png ") + os.path.join(outputdir,"Transcript_Log2length.boxplot.png ") + "&> " + os.path.join(outputdir,"transcript.length.exon.log\n\n"))
    fo.close()

def write_fpkm_density_makeflow(makeflow,outputdir,count,fpkm,samples):
    fo=open(makeflow,"a+")
    fo.write("CATEGORY=fpkm_density\n")
    fo.write(" ".join([os.path.join(outputdir,"expression",i+".fpkm_density.png") for i in samples]) + " : " + fpkm +"\n")
    fo.write("\tRscript /lustre/work/yongdeng/software/eukaryon/flow/fpkm_density_plot_func.r %s %s &> %s\n\n"%(fpkm,os.path.join(outputdir,"expression"),os.path.join(outputdir,"expression","genes.density.err")))
    fo.write("CATEGORY=fpkm_saturation\n")
    fo.write(" ".join([os.path.join(outputdir,"expression",i+".saturation.png") for i in samples]) + " : " + count + " " + fpkm +"\n")
    fo.write("\tRscript /lustre/work/yongdeng/software/eukaryon/flow/fpkm_saturation_v3.r --fpkm %s --count %s --o %s &> %s\n\n"%(fpkm,count,os.path.join(outputdir,"expression"),os.path.join(outputdir,"expression","genes.saturation.err")))
    fo.close()

def get_matrix_anno(makeflow,anno,gtf,counttable,matrixtype):
    fo = open(makeflow,"a+")
    newname = os.path.splitext(counttable)[0] + ".anno.txt"
    fo.write("CATEGORY=matrxi_anno_%s\n"%os.path.basename(counttable))
    fo.write(newname + " : " + counttable + "\n")
    fo.write("\tpython /lustre/work/yongdeng/software/eukaryon/flow/matrix_anno.py -t %s -anno %s -gtf %s -type %s -o %s &> %s\n\n"%(counttable,anno,gtf,matrixtype,newname,counttable+".anno.log"))
    fo.close()

def violin_plot(makeflow,fpkmtab,outpng):
    fo = open(makeflow,"a+")
    fo.write("CATEGORY=violin_plot\n")
    fo.write(os.path.dirname(fpkmtab) + "/" + outpng + " : " + fpkmtab + "\n")
    fo.write("\tpython /lustre/work/yongdeng/software/eukaryon/flow/draw_violin.py -i %s -o %s &> %s\n\n"%(fpkmtab,os.path.dirname(fpkmtab) + "/" + outpng,os.path.dirname(fpkmtab) + "/" + outpng + ".log"))
    
def parse_genetrans_info_anno(makeflow,gtf,anno,outdir):
    fo = open(makeflow,"a+")
    fo.write("CATEGORY=parse_genetrans_info_anno_back_list\n")
    fo.write(os.path.join(outdir,"track_ref.geneanno.txt ") + os.path.join(outdir,"track_ref.transanno.txt ") + os.path.join(outdir,"back.list : ") + gtf + " " + anno + "\n")
    fo.write("\tpython /lustre/work/yongdeng/software/protokaryon/flow/parse_genetrans_info_anno.py -gtf %s -anno %s -b %s -g %s -t %s &> %s\n\n"%(gtf.replace(".orl",""),anno,os.path.join(outdir,"back.list"),os.path.join(outdir,"track_ref.geneanno.txt"),os.path.join(outdir,"track_ref.transanno.txt"),os.path.join(outdir,"parse_genetrans_back.log")))
...
def main():
    ...
    if assemble:
        mkdir(assembledir,samples + ["merge","expression"])
        ...
        write_trans_length_exon_num(args.makeflow,os.path.join(assembledir,"merge/merged.gtf"),assembledir,os.path.join(assembledir,"transcript_count_matrix.txt"))
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),os.path.join(assembledir,"merge/merged.gtf"),os.path.join(assembledir,"gene_count_matrix.txt"),"gene")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),os.path.join(assembledir,"merge/merged.gtf"),os.path.join(assembledir,"gene_FPKM_matrix.txt"),"gene")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),os.path.join(assembledir,"merge/merged.gtf"),os.path.join(assembledir,"transcript_FPKM_matrix.txt"),"trans")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),os.path.join(assembledir,"merge/merged.gtf"),os.path.join(assembledir,"transcript_count_matrix.txt"),"trans")
        parse_genetrans_info_anno(args.makeflow,os.path.join(assembledir,"merge/merged.gtf.orl"),os.path.abspath(args.anno),assembledir)

    else:
        mkdir(assembledir,samples+["expression"])
        ...
        write_trans_length_exon_num(args.makeflow,gtf,assembledir,os.path.join(assembledir,"transcript_count_matrix.txt"))
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),gtf,os.path.join(assembledir,"gene_count_matrix.txt"),"gene")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),gtf,os.path.join(assembledir,"gene_FPKM_matrix.txt"),"gene")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),gtf,os.path.join(assembledir,"transcript_FPKM_matrix.txt"),"trans")
        get_matrix_anno(args.makeflow,os.path.abspath(args.anno),gtf,os.path.join(assembledir,"transcript_count_matrix.txt"),"trans")
        parse_genetrans_info_anno(args.makeflow,gtf,os.path.abspath(args.anno),assembledir)
    violin_plot(args.makeflow,os.path.join(assembledir,"transcript_FPKM_matrix.txt"),"trans.violin.png")
    violin_plot(args.makeflow,os.path.join(assembledir,"gene_FPKM_matrix.txt"),"genes.violin.png")
    
    write_fpkm_density_makeflow(args.makeflow,assembledir,os.path.join(assembledir,"gene_count_matrix.txt"),os.path.join(assembledir,"gene_FPKM_matrix.txt"),samples)
