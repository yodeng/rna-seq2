...
def write_get_count_matrix_makeflow(makeflow,samples,assembledir):
    ...
    fo.write(os.path.join(assembledir,"gene_count_matrix.txt.all ") + os.path.join(assembledir,"transcript_count_matrix.txt.all : ") + " ".join([os.path.join(assembledir,sn,"transcripts.gtf") for sn in samples]) + "\n")
    fo.write("\tpython %s -i %s -g %s -t %s &> %s\n\n"%(prepDE,assembledir,os.path.join(assembledir,"gene_count_matrix.txt.all"),os.path.join(assembledir,"transcript_count_matrix.txt.all"),os.path.join(assembledir,"get_count_matrix.log")))
    fo.close()
...
def write_get_expression_matrix_makeflow(makeflow,samples,assembledir,type):
    ...
    fo.write(os.path.join(assembledir,"gene.expression.txt.all : ") + " ".join([os.path.join(assembledir,sn,"gene_abundance.txt") for sn in samples]) + "\n")
    fo.write("\tpython %s -i %s -o %s -t %s &> %s\n\n"%(get_expression_matrix," ".join([os.path.join(assembledir,sn,"gene_abundance.txt") for sn in samples]),os.path.join(assembledir,"gene.expression.txt.all"),type,os.path.join(assembledir,"get_gene_expression.log")))
    fo.write("CATEGORY=get_isoform_expression_matrix\n")
    fo.write(os.path.join(assembledir,"isoform.expression.txt.all : ") + " ".join([os.path.join(assembledir,sn,"transcripts.gtf.filter.gtf") for sn in samples]) + "\n")
    fo.write("\tpython %s -i %s -o %s -t %s &> %s\n\n"%(get_expression_matrix," ".join([os.path.join(assembledir,sn,"transcripts.gtf.filter.gtf") for sn in samples]),os.path.join(assembledir,"isoform.expression.txt.all"),type,os.path.join(assembledir,"get_isoform_expression.log")))
    fo.close()
...
def write_filter_feature_makeflow(makeflow,assembledir,gtf,assemble):
    ...
    fo.write(" ".join([os.path.join(assembledir,i) for i in ["gene_count_matrix.txt","gene.expression.txt","isoform.expression.txt","transcript_count_matrix.txt"]]) + " : " + " ".join([os.path.join(assembledir,i) for i in ["gene_count_matrix.txt.all","gene.expression.txt.all","isoform.expression.txt.all","transcript_count_matrix.txt.all"]]) + "\n")
    if assemble:
        fo.write("\tpython %s %s %s\n\n"%(filter_new_feature,gtf," ".join([os.path.join(assembledir,i) for i in ["gene_count_matrix.txt.all","gene.expression.txt.all","isoform.expression.txt.all","transcript_count_matrix.txt.all"]])))
    else:
...
def main():
    ...
    write_filter_feature_makeflow(args.makeflow,assembledir,gtf,assemble)
